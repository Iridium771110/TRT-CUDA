...
关于混合专家模型  
理解上相当于是多了一个训练出来的分派网络，通过分派网络（又称 路由？）将任务分派到不同的专家模型中（可以理解为一种学习型的if-else方案）。  
在一般的实现中，attention的结构往往是共享的（类似于编码特征的理解），而后续的FFN网络将成为混合专家模型所在，进行分派式推理减少计算量。实现中可能（or 大概率）使用分划形式，将特征的不同部分使用不同的专家模型进行计算，结束后再将输出拼接成传统的正常共同计算形式下的输出格式。->这里可能会有一部分类似隐式等的操作  
